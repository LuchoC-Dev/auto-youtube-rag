...
exist (`auto-design` and `catalog-design`, the latter with a few more videos),
...
`catalog-design` is noted as an additional candidate for block T. If this
...
- **Block T** — real E2E: copy the `auto-design` collection (and, time
  permitting, `catalog-design`), synchronise including the videos with
...
3. **SQLite migration:** path 2 — edit `001-initial.ts` in place to include
   ...
   exists to preserve; `auto-design` and `catalog-design` are source
